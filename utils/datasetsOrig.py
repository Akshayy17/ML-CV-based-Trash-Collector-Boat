...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        for i, s in enumerate(sources):
            ...
            #print("datasets: loop to find number of sources")
    ...
    def update(self, index, cap):
        ...
        n = 0
        #print(f'datasets: def update {n}')

        while cap.isOpened():
            n += 1
            #print(f'datasets: def update while-1 {n} {time.time():.4f} s')

            # _, self.imgs[index] = cap.read()
            cap.grab()
            if n == 1:  # read every 4th frame --- akshay
                _, self.imgs[index] = cap.retrieve()
                #print(f'datasets: def update while-if {index}')
                n = 0
            time.sleep(0.01)  # wait time --- original 0.01 akshay

    def __iter__(self):
        self.count = -1
        #print(f'datasets: def __iter__')
        return self

    def __next__(self):
        self.count += 1
        img0 = self.imgs.copy()
        #print(f'datasets: def __next__ {time.time():.4f} s')
